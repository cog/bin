...
my @aliases = map { s/alias\s+([^=]+)/$1/; $1 } grep {/^alias/} slurp( $alias_file );
...
sub sort_aliases {
    ...
    return (sort { $order * ($aliases->{$a}->{ $sort_param } <=> $aliases->{$b}->{ $sort_param }) } keys %$aliases)[ 0 .. $size ];
}
...
sub display_selection {
    ...
    for my $key ( @keys ) {
        print "    $key - $aliases->{$key}->{'total'} times, $aliases->{$key}->{'last_used'} commands ago\n"
    }
